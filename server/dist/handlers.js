import { GamePhase, CardType, initializeGame, placeBid, updateGameState, chooseEscapeOrPirate, continueTrick, continueRound, isValidPlay, chooseBid, chooseCard, chooseTigress, getBotName, } from "@cool-king/engine";
import { createRoom, joinRoom, leaveRoom, getRoom, updateRoom, sanitizeState, touchRoom, markDisconnected, reconnectPlayer, replaceWithBot, getRoomByPlayerId, getPlayerIdBySocket, getSocketId, addBot, removeBot, } from "./rooms.js";
...
function broadcastState(io, roomId) {
    ...
    for (const player of state.players) {
        if (player.isBot)
            continue; // bots don't need state
...
function scheduleBotTurn(io, roomId) {
    ...
    if (!currentPlayer?.isBot || !currentPlayer.botDifficulty)
        return;
...
function startBidTimer(io, roomId) {
    clearBidTimer(roomId);
    const timer = setTimeout(() => {
        bidTimers.delete(roomId);
        let state = getRoom(roomId);
        if (!state || state.phase !== GamePhase.BIDDING)
            return;
...
function clearScoringTimer(roomId) {
...
function startScoringTimer(io, roomId) {
    clearScoringTimer(roomId);
    const timer = setTimeout(() => {
        scoringTimers.delete(roomId);
        let state = getRoom(roomId);
        if (!state || state.phase !== GamePhase.ROUND_SCORING)
            return;
...
function scheduleBotBids(io, roomId) {
    const state = getRoom(roomId);
    if (!state || state.phase !== GamePhase.BIDDING)
        return;
    ...
    bots.forEach((bot, i) => {
        ...
        setTimeout(() => {
            ...
            if (!current || current.phase !== GamePhase.BIDDING)
                return;
            const currentBot = current.players.find((p) => p.id === bot.id);
            if (!currentBot?.isBot || !currentBot.botDifficulty || currentBot.bid !== undefined)
                return;
            try {
                ...
                console.error(`[bot-bid-error] ${bot.name}: ${err.message}`);
            }
        }, delay);
    });
}
...
function requireTurn(ctx, phase) {
    if (ctx.room.phase !== phase)
        throw new Error(`Not in ${phase} phase`);
    if (ctx.room.currentPlayer !== ctx.playerId)
...
function startDisconnectTimer(io, roomId, playerId) {
...
function triggerBotReplacement(io, roomId, playerId) {
...
export function registerHandlers(io) {
    io.on("connection", (socket) => {
        ...
        const emitError = (msg) => socket.emit("error", { message: msg });
        const handle = (fn) => {
            try {
                fn();
            }
        ...
        // ─── Create Room ───────────────────────────────
        socket.on("create-room", ({ playerName }) => handle(() => {
            if (!playerName?.trim())
        ...
        // ─── Join Room ─────────────────────────────────
        socket.on("join-room", ({ roomId, playerName }) => handle(() => {
            if (!playerName?.trim())
                throw new Error("Name is required");
            if (!roomId?.trim())
        ...
        // ─── Add Bot ───────────────────────────────────
        socket.on("add-bot", ({ difficulty }) => handle(() => {
        ...
        // ─── Remove Bot ────────────────────────────────
        socket.on("remove-bot", ({ botId }) => handle(() => {
            const { room, playerId } = requireRoom(socket.id);
            const player = room.players.find((p) => p.id === playerId);
            if (!player?.isHost)
        ...
        // ─── Reconnect ─────────────────────────────────
        socket.on("reconnect-room", ({ roomId, playerId }) => handle(() => {
        ...
        socket.on("place-bid", ({ bid }) => handle(() => {
            const { room, playerId } = requireRoom(socket.id);
        ...
        // ─── Play Card ─────────────────────────────────
        socket.on("play-card", ({ cardId }) => handle(() => {
        ...
        // ─── Choose Tigress ────────────────────────────
        socket.on("choose-tigress", ({ choice }) => handle(() => {
        ...
        socket.on("continue-trick", () => handle(() => {
            ...
            if (room.phase !== GamePhase.TRICK_RESULT)
        ...
        socket.on("continue-round", () => handle(() => {
            const { room } = requireRoom(socket.id);
            if (room.phase !== GamePhase.ROUND_SCORING)
