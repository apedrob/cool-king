import { CardType, GamePhase, } from "./types.js";
 ...
 */
export function nextPlayer(playerId, players) {
 ...
 */
export function initializeGame(players, maxRounds) {
...
export function placeBid(state, playerId, bid) {
    ...
    const startingPlayer = state.players[(state.currentRound - 1) % state.players.length].id;
    return {
        ...state,
        players,
 ...
 */
export function updateGameState(state, playerId, card) {
 ...
 */
function calculateTrickBonuses(trick, winnerId) {
...
function resolveTrick(state) {
    ...
    const players = state.players.map((p) => p.id === winner ? { ...p, tricks: p.tricks + 1 } : p);
 ...
 */
export function continueTrick(state) {
 ...
 */
export function chooseEscapeOrPirate(state, choice) {
