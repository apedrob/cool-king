...
export function getBotName(difficulty: Difficulty): string {
...
function getValidCards(player: Player, leadColor?: CardColor): Card[] {
...
function countSpecials(hand: Card[]): number {
    return hand.filter((c) =>
        c.type === CardType.PIRATE ||
        c.type === CardType.MERMAID ||
        c.type === CardType.SKULL_KING ||
        c.type === CardType.TIGRESS
    ).length;
}

function countTrumps(hand: Card[]): number {
    return hand.filter((c) => c.type === CardType.NUMBERED && c.color === CardColor.BLACK).length;
}

function countHighCards(hand: Card[]): number {
    return hand.filter((c) =>
        c.type === CardType.NUMBERED && c.value !== undefined && c.value >= 10
    ).length;
}

// ─── Bidding AI ──────────────────────────────────────

export function chooseBid(player: Player, round: number, difficulty: Difficulty): number {
    switch (difficulty) {
        case "easy":
        ...
        case "hard": {
            ...
            const estimate =
                skBonus + mermaidBonus + pirateCount +
...
export function chooseCard(player: Player, state: GameState, difficulty: Difficulty): Card {
...
function chooseMediumCard(player: Player, state: GameState, valid: Card[]): Card {
    ...
    if (wantToWin) {
        ...
        if (specials.length > 0) return randomItem(specials);
    ...
    if (wantToLose) {
        ...
        if (escapes.length > 0) return escapes[0];
...
function chooseHardCard(player: Player, state: GameState, valid: Card[]): Card {
    const wantToWin = player.bid !== undefined && player.tricks < player.bid;
    const wantToLose = player.bid !== undefined && player.tricks >= player.bid;
    ...
    const _trickHasPirate = state.currentTrick.some((t) =>
        t.card.type === CardType.PIRATE || (t.card.type === CardType.TIGRESS && t.card.chosenType === CardType.PIRATE)
    );
    ...
    if (wantToWin) {
        ...
        if (isLeading) {
            ...
            if (trumps.length > 0) return trumps.sort((a, b) => (b.value || 0) - (a.value || 0))[0];
        }
        ...
        const numbered = valid.filter((c) => c.type === CardType.NUMBERED);
        if (numbered.length > 0) return numbered.sort((a, b) => (b.value || 0) - (a.value || 0))[0];
    }
    ...
    if (wantToLose) {
        ...
        const offSuit = valid.filter((c) =>
            c.type === CardType.NUMBERED && c.color !== CardColor.BLACK && c.color !== state.leadColor
        );
        if (offSuit.length > 0) return offSuit.sort((a, b) => (a.value || 0) - (b.value || 0))[0];

        // Play lowest card overall
        const numbered = valid.filter((c) => c.type === CardType.NUMBERED);
        if (numbered.length > 0) return numbered.sort((a, b) => (a.value || 0) - (b.value || 0))[0];
    }
...
export function chooseTigress(player: Player, state: GameState, difficulty: Difficulty): CardType.ESCAPE | CardType.PIRATE {
    switch (difficulty) {
        case "easy":
