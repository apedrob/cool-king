 ...
 */
function effectiveType(card: Card): CardType {
 ...
 */
export function determineWinner(trick: TrickPlay[], _leadColor?: CardColor): string {
    ...
    let bestIndex = 0;
    let bestValue = -1;
    let bestColor: CardColor | undefined;

    for (let i = 0; i < trick.length; i++) {
        const card = trick[i].card;
        const type = types[i];

        if (type === CardType.ESCAPE) continue;
 ...
 */
export function isValidPlay(card: Card, hand: Card[], leadColor?: CardColor): boolean {
    // No lead color yet — anything goes
    if (!leadColor) return true;

    // Special cards can always be played
    if (card.type !== CardType.NUMBERED) return true;
    ...
    const hasLeadColor = hand.some(
        (c) => c.type === CardType.NUMBERED && c.color === leadColor
    );
    return !hasLeadColor;
}
